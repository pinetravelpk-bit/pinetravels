...
/** Admin: saare hotels (published + unpublished). DB na ho to file se. */
export async function listHotelsAdmin() {
  if (dbConfigured()) {
    const rows = await query("SELECT slug, data FROM hotels ORDER BY sort_order ASC, id ASC");
    if (rows && rows.length) return rows.map((r) => ({ ...parseJSON(r.data, {}), slug: r.slug }));
  }
  return fileHotels;
}

/** Admin: aik hotel edit ke liye. DB me na ho to file se le aata hai
 *  (save karte hi DB me row ban jati hai). */
export async function getHotelForAdmin(slug) {
  if (dbConfigured()) {
    const row = await queryOne("SELECT slug, data FROM hotels WHERE slug = ? LIMIT 1", [slug]);
    if (row) return { ...parseJSON(row.data, {}), slug: row.slug };
  }
  return fileHotels.find((h) => h.slug === slug) || null;
}

/** Admin: hotel save/update (slug column se, JSON me slug nahi rakhte). */
export async function setHotel(slug, data) {
  const clean = { ...data };
  delete clean.slug;
  await query(
    `INSERT INTO hotels (slug, data, published) VALUES (?, ?, 1)
     ON DUPLICATE KEY UPDATE data = VALUES(data)`,
